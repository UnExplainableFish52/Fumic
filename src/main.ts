...
type AppView = "home" | "settings" | "info" | "player";
...
function handleKeyboard(event: KeyboardEvent) {
  ...
  if (event.key === "Escape" && state.activeView === "player") {
    event.preventDefault();
    switchView("home");
    return;
  }
...
async function playMediaItem(item: MediaLibraryItem) {
  ...
  if (item.kind === "video") {
    /* Auto-switch to fullscreen player so the video is visible */
...
async function playByIndex(tab: LibraryTab, index: number) {
  ...
  if (state.activeView !== "home" && state.activeView !== "player") {
    state.activeView = "home";
    renderShell();
  } else {
    updatePlayingIndicator();
...
function getRuntimeBadgeText() {
  if (!bootstrap) {
    return "Loading runtime info...";
  }

  return `${bootstrap.appName} ${bootstrap.appVersion} on ${bootstrap.platform}`;
}
